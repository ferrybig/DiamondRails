...
public class DiamondRails extends JavaPlugin implements Listener {
    ...
    public void onMinecartMove(VehicleMoveEvent event) {
        if (event.getVehicle() instanceof Minecart) {
            ...
            Block search = loc.getBlock();
            // Duo to gravity changes, the minecart may hover down
            Block belowSearch = search.getRelative(0, 1, 0);
            if (search.getType() == TRACK_MATERIAL
                    || belowSearch.getType() == TRACK_MATERIAL) {
